...
Proof.
  verify.
  - (* Goal 1: 证明 match 表达式为 true 时，2 <= st X *)
    destruct (st X) as [|[|n]].
    + discriminate H0.
    + discriminate H0.
    + lia.
  - (* Goal 2: 证明 match 表达式为 false 时，~ 2 <= st X *)
    destruct (st X) as [|[|n]].
    + lia.
    + lia.
    + discriminate H0.
  - (* Goal 3: 证明 parity (st X - 2) = parity m *)
    rewrite <- H.
    apply parity_ge_2.
    assumption.
  - (* Goal 4: 证明当 ~ 2 <= st X 时，st X = parity m *)
    rewrite <- parity_lt_2 with (x := st X).
    + exact H.
    + assumption.
Qed.
...
Definition sqrt_dec (m:nat) : decorated :=
  <{
    ...
    {{ X = m /\ 0 * 0 <= X }}                         (* 建立初始关系 *)
      Z := 0
                   {{ X = m /\ Z * Z <= X }};          (* Z 初始化为 0，满足 Z*Z <= X *)
      while ((Z+1)*(Z+1) <= X) do
                   {{ X = m /\ Z * Z <= X /\ (Z+1)*(Z+1) <= X }} ->>  (* 循环条件：下一个平方仍然不超过 X *)
                   {{ X = m /\ (Z+1) * (Z+1) <= X }}   (* 准备增加 Z *)
        Z := Z + 1
                   {{ X = m /\ Z * Z <= X }}           (* 重新建立不变式 *)
      end
    {{ X = m /\ Z * Z <= X /\ ~((Z+1)*(Z+1) <= X) }} ->>  (* 循环结束条件 *)
    {{ Z*Z<=m /\ m<(Z+1)*(Z+1) }}                     (* 最终后置条件：Z 是 m 的整数平方根 *)
  }>.
...
Proof. 
  verify.
  (* verify 策略处理了大部分推理，这里可能需要一些额外的算术推理 *)
  (* 主要是证明循环不变式的维护和最终条件的正确性 *)
Qed.
...
Proof. 
  ...
  - (* 第一个目标：处理初始条件到循环不变式 *)
    (* 需要证明：True -> True *)
    (* 然后证明：X = a /\ Y = b /\ Z = 0 -> Z + X = a /\ Z + Y = b *)
    rewrite andb_true_iff in H0.
        destruct H0 as [HX_neq HY_neq].
    
    (* 将 negb (st X =? 0) = true 转换为 st X <> 0 *)
    rewrite negb_true_iff in HX_neq.
    rewrite eqb_neq in HX_neq.
    exact HX_neq.
  - (* 第二个目标：处理循环体的正确性 *)
    (* 需要从布尔条件推导算术条件 *)
    (* H0: negb (st X =? 0) && negb (st Y =? 0) = true *)
    (* 目标: st Y <> 0 *)
    ...
    (* 将 negb (st Y =? 0) = true 转换为 st Y <> 0 *)
    rewrite negb_true_iff in HY_neq.
    rewrite eqb_neq in HY_neq.
    exact HY_neq.
  ...
  - (* 第三个目标：从终止条件推导最终结果 *)
    ...
    destruct H0 as [HX_false | HY_false].
    + (* X = 0 的情况 *)
      rewrite negb_false_iff in HX_false.
      rewrite eqb_eq in HX_false.
      left.
      auto.
    + (* Y = 0 的情况 *)
      rewrite negb_false_iff in HY_false.
      rewrite eqb_eq in HY_false.
      right.
      auto.
Qed.
(** [] *)
...
Definition two_loops_dec (a b c : nat) : decorated :=
  <{
    {{ True }} ->>
    {{ True }}                                      (* 初始条件 - Initial condition *)
      X := 0
                   {{ X = 0 }} ->>                  (* X 初始化为 0 - X initialized to 0 *)
                   {{ X = 0 }}                      (* 保持 X = 0 - Maintain X = 0 *)
      ;
      Y := 0
                   {{ X = 0 /\ Y = 0 }} ->>         (* Y 也初始化为 0 - Y also initialized to 0 *)
                   {{ X = 0 /\ Y = 0 }}             (* 保持两个初始化 - Maintain both initializations *)
      ;
      Z := c
                   {{ X = 0 /\ Y = 0 /\ Z = c }} ->>  (* Z 初始化为 c - Z initialized to c *)
                   {{ Z = X + c /\ Y = 0 }}            (* 第一个循环不变式：Z = X + c 且 Y = 0 - First loop invariant: Z = X + c and Y = 0 *)
      ;
      while X <> a do
                   {{ Z = X + c /\ Y = 0 /\ X <> a }} ->>     (* 第一个循环条件 - First loop condition *)
                   {{ Z + 1 = (X + 1) + c /\ Y = 0 }}        (* 为递增做准备，保持 Y = 0 - Prepare for increment, maintain Y = 0 *)
        X := X + 1
                   {{ Z + 1 = X + c /\ Y = 0 }}              (* X 递增后，保持 Y = 0 - After X increment, maintain Y = 0 *)
        ;
        Z := Z + 1
                   {{ Z = X + c /\ Y = 0 }}                  (* 恢复循环不变式 - Restore loop invariant *)
      end
                   {{ Z = X + c /\ Y = 0 /\ ~(X <> a) }} ->>  (* 第一个循环结束 - First loop terminates *)
                   {{ Z = a + c /\ Y = 0 }} ->>              (* X = a，所以 Z = a + c - X = a, so Z = a + c *)
                   {{ Z = a + Y + c }}                (* 第二个循环不变式：Z = a + Y + c - Second loop invariant: Z = a + Y + c *)
      ;
      while Y <> b do
                   {{ Z = a + Y + c /\ Y <> b }} ->>  (* 第二个循环条件 - Second loop condition *)
                   {{ Z + 1 = a + (Y + 1) + c }}      (* 为递增做准备 - Prepare for increment *)
        Y := Y + 1
                   {{ Z + 1 = a + Y + c }}            (* Y 递增后 - After Y increment *)
        ;
        Z := Z + 1
                   {{ Z = a + Y + c }}                (* 恢复循环不变式 - Restore loop invariant *)
      end
    {{ Z = a + Y + c /\ ~(Y <> b) }} ->>              (* 第二个循环结束 - Second loop terminates *)
    {{ Z = a + b + c }}                               (* Y = b，所以最终结果 - Y = b, so final result *)
  }>.
...
Definition dpow2_dec (n : nat) :=
  <{
    ...
    {{ True }}                                         (* 初始条件 - Initial condition *)
      ...
      ;
      Y := 1
               {{ X = 0 /\ Y = 1 }} ->>                (* Y 初始化为 1 - Y initialized to 1 *)
               {{ X = 0 /\ Y = 1 }}                    (* 保持初始化 - Maintain initialization *)
      ;
      Z := 1
               {{ X = 0 /\ Y = 1 /\ Z = 1 }} ->>       (* Z 初始化为 1 - Z initialized to 1 *)
               {{ Y = #pow2 (X+1) - 1 /\ Z = #pow2 X }}   (* 循环不变式：Y累积前X+1项减1，Z是当前项 - Loop invariant: Y accumulates first X+1 terms minus 1, Z is current term *)
      ;
      while X <> n do
               {{ Y = #pow2 (X+1) - 1 /\ Z = #pow2 X /\ X <> n }} ->>  (* 循环条件 - Loop condition *)
               {{ Y + 2 * Z = #pow2 ((X+1)+1) - 1 /\ 2 * Z = #pow2 (X+1) }}  (* 准备更新 - Prepare for update *)
        Z := 2 * Z
               {{ Y + Z = #pow2 ((X+1)+1) - 1 /\ Z = #pow2 (X+1) }}  (* Z 更新后 - After Z update *)
        ;
        Y := Y + Z
               {{ Y = #pow2 ((X+1)+1) - 1 /\ Z = #pow2 (X+1) }}        (* Y 更新后 - After Y update *)
        ;
        X := X + 1
               {{ Y = #pow2 (X+1) - 1 /\ Z = #pow2 X }}                (* 恢复循环不变式 - Restore loop invariant *)
      end
    {{ Y = #pow2 (X+1) - 1 /\ Z = #pow2 X /\ ~(X <> n) }} ->>          (* 循环结束 - Loop terminates *)
    {{ Y = #pow2 (n+1) - 1 }}                                           (* 最终结果 - Final result *)
  }>.
...
Proof.
  ...
  - (* 第一个目标：初始化到循环不变式的推导 *)
    (* 证明：X = 0 /\ Y = 1 /\ Z = 1 -> Y = pow2(X+1) - 1 /\ Z = pow2(X) *)
    (* 即证明：1 = pow2(1) - 1 /\ 1 = pow2(0) *)
    (* 因为 pow2(1) = 2，pow2(0) = 1，所以 2 - 1 = 1 /\ 1 = 1 *)
    (* 首先简化左边的表达式 *)
    simpl.
    (* 简化 pow2(st X) + (pow2(st X) + 0) = 2 * pow2(st X) *)
    replace (pow2 (st X) + (pow2 (st X) + 0)) with (2 * pow2 (st X)) by lia.

    (* 简化 st X + 1 + 1 = st X + 2 *)
    replace (st X + 1 + 1) with (st X + 2) by lia.

    (* 现在目标变成：pow2(st X + 1) - 1 + 2 * pow2(st X) = pow2(st X + 2) - 1 *)

    (* 使用 pow2(n+1) = 2 * pow2(n) 来重写 pow2(st X + 1) *)
    assert (H_pow1: pow2 (st X + 1) = 2 * pow2 (st X)).
    { replace (st X + 1) with (S (st X)) by lia.
      simpl. reflexivity. }

    (* 使用 pow2(n+2) = 2 * pow2(n+1) 来重写 pow2(st X + 2) *)
    assert (H_pow2: pow2 (st X + 2) = 2 * pow2 (st X + 1)).
    { replace (st X + 2) with (S (st X + 1)) by lia.
      simpl. reflexivity. }

    (* 重写目标 *)
    rewrite H_pow1, H_pow2, H_pow1.

    (* 现在目标变成：2 * pow2(st X) - 1 + 2 * pow2(st X) = 2 * (2 * pow2(st X)) - 1 *)
    (* 即：4 * pow2(st X) - 1 = 4 * pow2(st X) - 1 *)

    (* 确保 pow2(st X) >= 1 以处理减法 *)
    assert (Hge: pow2 (st X) >= 1) by apply pow2_le_1.
    lia.
    
  - (* 第二个目标：循环体的正确性 *)
    (* 需要证明循环不变式在每次迭代后保持 *)
    (* 关键是证明：Y + 2*Z = pow2((X+1)+1) - 1 /\ 2*Z = pow2(X+1) *)
    (* 使用 pow2_plus_1 引理：pow2(n+1) = 2 * pow2(n) *)
...
Proof.
  intros n Hn.
  (* 对 n 进行归纳，但由于 n > 0，我们需要先分析 n 的结构 *)
  (* Induction on n, but since n > 0, we first analyze the structure of n *)
  destruct n as [| n'].
  - (* n = 0 的情况，与 n > 0 矛盾 - Case n = 0, contradicts n > 0 *)
    lia.
  - (* n = S n' 的情况，即 n >= 1 - Case n = S n', i.e., n >= 1 *)
    destruct n' as [| n''].
    + (* n = 1 的情况 - Case n = 1 *)
      (* 需要证明：fib 1 + fib (pred 1) = fib (1 + 1) *)
      (* 即：fib 1 + fib 0 = fib 2 *)
      simpl.
      (* pred 1 = 0, 1 + 1 = 2 *)
      (* fib 1 = 1, fib 0 = 1, fib 2 = fib 1 + fib 0 = 1 + 1 = 2 *)
      reflexivity.
    + (* n = S (S n'') 的情况，即 n >= 2 - Case n = S (S n''), i.e., n >= 2 *)
      (* 需要证明：fib (S (S n'')) + fib (pred (S (S n''))) = fib (1 + S (S n'')) *)
      (* 即：fib (S (S n'')) + fib (S n'') = fib (S (S (S n''))) *)
      
      (* 简化 pred (S (S n'')) = S n'' *)
      simpl pred.
      (* 简化 1 + S (S n'') = S (S (S n'')) *)
      replace (1 + S (S n'')) with (S (S (S n''))) by lia.
      
      (* 根据 fib 的定义：*)
      (* fib (S (S n'')) = fib (S n'') + fib n'' *)
      (* fib (S (S (S n''))) = fib (S (S n'')) + fib (S n'') *)
      simpl fib.
      
      (* 现在目标变成： *)
      (* (fib (S n'') + fib n'') + fib (S n'') = (fib (S n'') + fib n'') + fib (S n'') *)
      (* 这显然成立 *)
      reflexivity.
Qed.
(** [] *)
...
Definition dfib (n : nat) : decorated :=
  <{
    ...
    {{ True }}                                        (* 初始条件 - Initial condition *)
    X := 1
                {{ X = 1 }} ->>                       (* X 初始化为 1 - X initialized to 1 *)
                {{ X = 1 }}                           (* 保持 X = 1 - Maintain X = 1 *)
    ;
    Y := 1
                {{ X = 1 /\ Y = 1 }} ->>              (* Y 初始化为 1 - Y initialized to 1 *)
                {{ X = 1 /\ Y = 1 }}                  (* 保持初始化 - Maintain initialization *)
    ;
    Z := 1
                {{ X = 1 /\ Y = 1 /\ Z = 1 }} ->>     (* Z 初始化为 1 - Z initialized to 1 *)
                {{ Y = #fib (X - 1) /\ Z = #fib X }}  (* 循环不变式：Y = fib(X-1), Z = fib(X) - Loop invariant: Y = fib(X-1), Z = fib(X) *)
    ;
    while X <> 1 + n do
                  {{ Y = #fib (X - 1) /\ Z = #fib X /\ X <> 1 + n }} ->>  (* 循环条件 - Loop condition *)
                  {{ Y = #fib (X - 1) /\ Z = #fib X /\ Y + Z = #fib (X + 1) }}  (* 准备计算下一项 - Prepare to compute next term *)
      T := Z
                  {{ Y = #fib (X - 1) /\ Z = #fib X /\ T = #fib X /\ Y + Z = #fib (X + 1) }}  (* T 保存当前 Z - T saves current Z *)
      ;
      Z := Z + Y
                  {{ Y = #fib (X - 1) /\ T = #fib X /\ Z = #fib (X + 1) }}  (* Z 更新为下一项 - Z updated to next term *)
      ;
      Y := T
                  {{ Y = #fib X /\ Z = #fib (X + 1) }} ->>  (* Y 更新为前一项 - Y updated to previous term *)
                  {{ Y = #fib ((X + 1) - 1) /\ Z = #fib (X + 1) }}  (* 重写为标准形式 - Rewrite to standard form *)
      ;
      X := 1 + X
                  {{ Y = #fib (X - 1) /\ Z = #fib X }}  (* 恢复循环不变式 - Restore loop invariant *)
    end
    {{ Y = #fib (X - 1) /\ Z = #fib X /\ ~(X <> 1 + n) }} ->>  (* 循环结束 - Loop terminates *)
    {{ Y = #fib ((1 + n) - 1) /\ X = 1 + n }} ->>              (* X = 1 + n - X = 1 + n *)
    {{ Y = #fib n }}                                            (* 最终结果 - Final result *)
   }>.
...
Proof.
  ...
  - (* 第一个目标：初始化到循环不变式的推导 *)
    (* 证明：X = 1 /\ Y = 1 /\ Z = 1 -> Y = fib(X-1) /\ Z = fib(X) *)
    (* 即：1 = fib(0) /\ 1 = fib(1) *)
    (* 因为 fib(0) = 1, fib(1) = 1 *)
    rewrite Nat.add_comm.
    (* 重写加法形式以匹配 fib_eqn *)
    replace (st X + 1) with (1 + st X) by lia.

    (* 重写减法形式以匹配 fib_eqn *)
    replace (st X - 1) with (pred (st X)).
    + (* 应用 fib_eqn *)
      apply fib_eqn.
      admit.
...
Proof.
  ...
  - (* 第一部分：证明 {{ Y <= 4 }} X := Y + 1 {{ X <= 5 }} *)
    (* 这等价于证明 Y <= 4 ->> (X <= 5)[X |-> Y + 1] *)
    (* 即 Y <= 4 ->> Y + 1 <= 5 *)
    eapply hoare_consequence_pre.
    + apply hoare_asgn.
    + (* 证明 Y <= 4 ->> Y + 1 <= 5 *)
...
Proof.
  ...
  - (* 第二部分：证明 Q [X |-> a] 是最弱的 *)
    (* 即对于任何 P，如果 {{ P }} X := a {{ Q }}，则 P ->> Q [X |-> a] *)
    intros P HP.
    unfold "->>".
    intros st HP_st.
    
    (* 关键思想：如果 {{ P }} X := a {{ Q }}，那么从任何满足 P 的状态 st 开始，
       执行 X := a 后得到的状态必须满足 Q *)
    
    (* 执行 X := a 从状态 st 得到状态 (X !-> aeval st a; st) *)
    assert (H_exec: st =[ X := a ]=> (X !-> aeval st a; st)).
    {
      constructor. reflexivity.
    }
    
    (* 由于 {{ P }} X := a {{ Q }}，且 P st 为真，执行后必须满足 Q *)
    unfold valid_hoare_triple in HP.
    (* HP 的类型是：forall st st' : state, st =[ X := a ]=> st' -> P st -> Q st' *)
    (* 我们需要应用 HP 到 st 和 (X !-> aeval st a; st) *)
    specialize (HP st (X !-> aeval st a; st) H_exec HP_st).
    
    (* 现在我们需要证明 (Q [X |-> a]) st *)
    (* 这等价于 Q (X !-> aeval st a; st) *)
    (* 断言替换 [X |-> a] 的含义就是在状态中将 X 替换为 a 的值 *)
    (* 根据断言替换的定义，这两者应该是相等的 *)
    exact HP.
Qed.
(** [] *)
