...
(** 步骤确定性定理：证明小步语义的确定性
    如果一个表达式 x 能够步进到两个不同的结果 y1 和 y2，
    那么这两个结果必须相等。即：对于任何表达式，
    它的下一步计算结果是唯一确定的。 *)
Theorem step_deterministic :
  deterministic step.
Proof.
  unfold deterministic. intros x y1 y2 Hy1 Hy2.
  (* 展开确定性定义，引入假设：x步进到y1和y2 *)
  generalize dependent y2.
  induction Hy1; intros y2 Hy2.
  (* 对第一个步进推导进行归纳 *)
  
  - (* 情况1: ST_PlusConstConst - 两个常数相加 *)
    (* x = P (C n1) (C n2)，y1 = C (n1 + n2) *)
    inversion Hy2; subst.
    + (* 子情况1a: Hy2也是ST_PlusConstConst *)
      (* 两个推导都得到相同的结果 C (n1 + n2) *)
      reflexivity.
    + (* 子情况1b: Hy2是ST_Plus1 *)
      (* 这会要求C n1能够步进，但常数无法步进，产生矛盾 *)
      inversion H2.
    + (* 子情况1c: Hy2是ST_Plus2 *)
      (* 这会要求C n2能够步进，但常数无法步进，产生矛盾 *)
      inversion H3.
      
  - (* 情况2: ST_Plus1 - 左操作数步进 *)
    (* x = P t1 t2，t1步进到t1'，所以x步进到P t1' t2 *)
    inversion Hy2; subst.
    + (* 子情况2a: Hy2是ST_PlusConstConst *)
      (* 这要求t1是常数，但t1又能步进，产生矛盾 *)
      inversion Hy1.
    + (* 子情况2b: Hy2也是ST_Plus1 *)
      (* 两个推导都对t1进行步进，由归纳假设，结果相同 *)
      apply IHHy1 in H2. rewrite H2. reflexivity.
    + (* 子情况2c: Hy2是ST_Plus2 *)
      (* 这要求t1是值且t2能步进，但t1既是值又能步进，矛盾 *)
      inversion H1; subst. inversion Hy1.
      
  - (* 情况3: ST_Plus2 - 右操作数步进（左操作数已是值） *)
    (* x = P v1 t2，v1是值，t2步进到t2'，所以x步进到P v1 t2' *)
    inversion Hy2; subst.
    + (* 子情况3a: Hy2是ST_PlusConstConst *)
      (* 这要求t2是常数，但t2又能步进，产生矛盾 *)
      inversion Hy1.
    + (* 子情况3b: Hy2是ST_Plus1 *)
      (* 这要求v1能步进，但值无法步进，产生矛盾 *)
      inversion H; subst. inversion H3.
    + (* 子情况3c: Hy2也是ST_Plus2 *)
      (* 两个推导都对t2进行步进，由归纳假设，结果相同 *)
...
Proof.
  (* 我们构造一个反例：P (P (C 1) (C 2)) (C 3) *)
  exists (P (P (C 1) (C 2)) (C 3)).
  split.
  - (* 证明它是一个值 *)
    apply v_funny.
  - (* 证明它不是正常形式（即它能够步进） *)
    unfold normal_form. 
    intros H. apply H.
    (* 存在一个步进：P (P (C 1) (C 2)) (C 3) --> P (C 3) (C 3) *)
    exists (P (C 3) (C 3)).
    apply ST_Plus1.
    apply ST_PlusConstConst.
Qed.
...
Proof.
  ...
  - (* 证明 C 42 是一个值 *)
    apply v_const.
  - (* 证明 C 42 不是正常形式（即它能够步进） *)
    unfold normal_form.
    intros H. apply H.
    (* 存在一个步进：C 42 --> P (C 42) (C 0) *)
    exists (P (C 42) (C 0)).
    apply ST_Funny.
Qed.
...
Proof.
  ...
  - (* 证明它是正常形式（即不能步进） *)
    unfold normal_form.
    intros contra. destruct contra as [t' Hstep].
    (* 对可能的步进规则进行情况分析 *)
    inversion Hstep; subst.
    + (* ST_PlusConstConst: 要求右操作数是常数，但P (C 2) (C 3)不是 *)
      inversion H2.
Qed.
...
(* 不可证明 - 根据ST_IfTrue规则，这应该步进到(test tru tru tru)，不是tru *)
...
(* 可证明 - 根据ST_If规则，条件(test tru tru tru)步进到tru *)

(* 这里是命题3的实际证明 *)
Example bool_step_prop3_proof : bool_step_prop3.
Proof.
...
Proof.
  ...
  - (* t = test t1 t2 t3 *)
    ...
    + (* t1能够步进 *)
      right. destruct Hstep as [t1' Hstep1].
      exists (test t1' t2 t3). apply ST_If. apply Hstep1.
Qed.
(** [] *)
...
Proof.
  ...
  - (* ST_If *)
    inversion Hy2; subst.
    + (* ST_IfTrue *) inversion Hy1.
    + (* ST_IfFalse *) inversion Hy1.
    + (* ST_If *) apply IHHy1 in H3. rewrite H3. reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  (* 使用短路规则：test (test tru tru tru) fls fls --> fls *)
  apply ST_IfShortcut.
  (* 证明 fls 是一个值 *)
  apply v_fls.
Qed.
...
Proof.
  ...
  (* P11 : x -->* y1, P12 : step_normal_form y1 *)
  (* P21 : x -->* y2, P22 : step_normal_form y2 *)
  (* 我们需要证明 y1 = y2 *)
  
  (* 关键思路：我们将证明如果从同一个起点出发，通过多步归约到达两个不同的正常形式，
     那么这两个正常形式必须相等 *)
  
  (* 我们使用强归纳法在多步归约的长度上进行证明 *)
  assert (Lemma1: forall t nf1 nf2,
    t -->* nf1 -> step_normal_form nf1 ->
    t -->* nf2 -> step_normal_form nf2 ->
    nf1 = nf2).
  {
    intros t nf1 nf2 H1 Hnf1 H2 Hnf2.
    (* 对H1进行归纳 *)
    generalize dependent nf2.
    induction H1 as [|t t' nf1 Hstep H1' IH]; intros nf2 H2 Hnf2.
    - (* H1 = multi_refl: t = nf1 *)
      (* 所以 t -->* nf2，且 t 是正常形式 *)
      (* 对H2进行情况分析 *)
      inversion H2; subst.
      + (* H2 = multi_refl: t = nf2 *)
        reflexivity.
      + (* H2 = multi_step: t --> y --> ... --> nf2 *)
        (* 但是 t 是正常形式，不能步进，矛盾 *)
        unfold step_normal_form, normal_form in Hnf1.
        exfalso. apply Hnf1. exists y. assumption.
    - (* H1 = multi_step: t --> t' -->* nf1 *)
      (* 对H2进行情况分析 *)
      inversion H2; subst.
      + (* H2 = multi_refl: t = nf2 *)
        (* 但是 t --> t'，所以 t 不是正常形式，矛盾 *)
        unfold step_normal_form, normal_form in Hnf2.
        exfalso. apply Hnf2. exists t'. assumption.
      + (* H2 = multi_step: t --> y -->* nf2 *)
        (* 由于单步归约的确定性，t' = y *)
        assert (t' = y) as Heq.
...
Lemma multistep_congr_2 : forall v1 t2 t2',
     ...
     P v1 t2 -->* P v1 t2'.
Proof.
  intros v1 t2 t2' Hval H. induction H.
  - (* multi_refl *) apply multi_refl.
...
Proof.
  (* 对大步求值进行归纳 *)
  intros t n Heval.
  induction Heval as [n | t1 t2 n1 n2 H1 IH1 H2 IH2].
  ...
  - (* E_Plus: P t1 t2 ==> (n1 + n2) *)
    (* 从归纳假设得到：
       - t1 -->* C n1  (IH1)
       - t2 -->* C n2  (IH2)
       我们需要证明：P t1 t2 -->* C (n1 + n2)
    *)
    
    (* 第一阶段：使用合同性，P t1 t2 -->* P (C n1) t2 *)
    assert (Step1: P t1 t2 -->* P (C n1) t2).
    { apply multistep_congr_1. apply IH1. }
    
    (* 第二阶段：使用合同性，P (C n1) t2 -->* P (C n1) (C n2) *)
    assert (Step2: P (C n1) t2 -->* P (C n1) (C n2)).
    { apply multistep_congr_2.
      - apply v_const.
      - apply IH2. }
    
    (* 第三阶段：P (C n1) (C n2) --> C (n1 + n2) *)
    assert (Step3: P (C n1) (C n2) -->* C (n1 + n2)).
    { apply multi_R. apply ST_PlusConstConst. }
...
Proof.
  ...
  - (* ST_PlusConstConst: P (C n1) (C n2) --> C (n1 + n2) *)
    (* t = P (C n1) (C n2), t' = C (n1 + n2) *)
    (* 假设: C (n1 + n2) ==> n，所以 n = n1 + n2 *)
    inversion Heval; subst.
    (* 现在需要证明: P (C n1) (C n2) ==> n1 + n2 *)
    apply E_Plus.
    + apply E_Const.
    + apply E_Const.
    
  - (* ST_Plus1: P t1 t2 --> P t1' t2 其中 t1 --> t1' *)
    (* t = P t1 t2, t' = P t1' t2 *)
    (* 假设: P t1' t2 ==> n *)
    inversion Heval; subst.
    (* P t1' t2 ==> n1 + n2，其中 t1' ==> n1, t2 ==> n2 *)
    (* 需要证明: P t1 t2 ==> n1 + n2 *)
    apply E_Plus.
    + (* 需要证明: t1 ==> n1 *)
      (* 由归纳假设: t1 --> t1' -> t1' ==> n1 -> t1 ==> n1 *)
      apply IHHs. apply H1.
    + (* t2 ==> n2 *)
      apply H3.
      
  - (* ST_Plus2: P (C n1) t2 --> P (C n1) t2' 其中 t2 --> t2' *)
    (* t = P (C n1) t2, t' = P (C n1) t2' *)
    (* 假设: P (C n1) t2' ==> n *)
    inversion Heval; subst.
    (* P (C n1) t2' ==> n0 + n2，其中 C n1 ==> n0, t2' ==> n2 *)
    (* 需要证明: P (C n1) t2 ==> n0 + n2 *)
    apply E_Plus.
    + (* C n1 ==> n0，由于 C n1 ==> n0，必须有 n0 = n1 *)
      exact H2.
    + (* 需要证明: t2 ==> n2 *)
      (* 由归纳假设: t2 --> t2' -> t2' ==> n2 -> t2 ==> n2 *)
      apply IHHs. exact H4.
Qed.
(** [] *)
...
Proof.
  intros t t' Hnf.
  unfold normal_form_of in Hnf.
  destruct Hnf as [Hmulti Hnormal] .
  
  (* 首先证明 t' 是一个值 *)
  apply nf_same_as_value in Hnormal.
  
  (* 由于 t' 是值，它必须是常数 *)
  inversion Hnormal as [n Heq]; subst.
  (* 现在我们有 t' = C n *)
  exists n.
  split.
  - (* t' = C n *)
  ...
  - (* t ==> n *)
    (* 对多步归约进行归纳 *)
    induction Hmulti as [|t t'' Cn Hstep Hmulti' IH].
    + (* multi_refl: t = C n *)
      inversion Hnormal. subst.
      
      admit.
    + (* multi_step: t --> t'' -->* C n *)
      apply step__eval with t''.
      * exact Hstep.
      * apply IH. exact Hnormal.
Admitted.
(** [] *)
...
Proof.
  ...
  - (* evalF t = n -> t ==> n *)
    (* 首先证明一个更强的引理：对所有 t，都有 t ==> evalF t *)
    intro H. subst.
  ...
  - (* t ==> n -> evalF t = n *)
    (* 对求值关系进行归纳 *)
    intro H.
    induction H as [m | t1 t2 n1 n2 H1 IH1 H2 IH2].
    ...
    + (* E_Plus: P t1 t2 ==> n1 + n2 *)
      (* 需要证明 evalF (P t1 t2) = n1 + n2 *)
      (* 已知 t1 ==> n1, t2 ==> n2 *)
      simpl.
      (* evalF (P t1 t2) = evalF t1 + evalF t2 *)
      (* 由归纳假设：evalF t1 = n1, evalF t2 = n2 *)
...
Proof.
  ...
  intros x y1 y2 Hy1 Hy2.
  generalize dependent y2.
  induction Hy1; intros y2 Hy2; inversion Hy2; subst; try reflexivity.
  
  - (* ST_PlusConstConst vs ST_Plus1 *)
    (* P (C v1) (C v2) 不能通过 ST_Plus1 步进，因为 C v1 不能步进 *)
    inversion H2.
    
  - (* ST_PlusConstConst vs ST_Plus2 *)
    (* P (C v1) (C v2) 不能通过 ST_Plus2 步进，因为 C v2 不能步进 *)
    inversion H3.
    
  - (* ST_Plus1 vs ST_PlusConstConst *)
    (* 如果 P t1 t2 通过 ST_Plus1 步进，那么 t1 不是常数 *)
    inversion Hy1.
    
  - (* ST_Plus1 vs ST_Plus1 *)
    (* 两个都使用 ST_Plus1，由归纳假设确定性成立 *)
  ...
  - (* ST_Plus2 vs ST_PlusConstConst *)
    (* 如果通过 ST_Plus2，那么 t2 不是常数 *)
    inversion Hy1.
  ...
  - (* ST_Plus2 vs ST_Plus2 *)
    (* 两个都使用 ST_Plus2，由归纳假设确定性成立 *)
    f_equal. apply IHHy1. assumption.
    
  - (* ST_IfTrue vs ST_IfFalse *)
    (* tru ≠ fls *)
    inversion H3.
...
Proof.
  ...
  (* 反例：P tru (C 0) *)
  (* 这个项既不是值，也无法步进 *)
  assert (H1 : ~ value (P tru (C 0))).
  { unfold not. intros Hv. inversion Hv. }
  
  assert (H2 : ~ (exists t', P tru (C 0) --> t')).
  { unfold not. intros [t' Hs]. 
    inversion Hs; subst.
    - (* ST_PlusConstConst: 需要两个常数，但 tru 不是常数 *)
      inversion H4.
    - (* ST_Plus1: 需要 tru 能步进，但 tru 是值无法步进 *)
      inversion H5.
  }
  
  (* 应用假设 H 到反例 *)
  specialize (H (P tru (C 0))).
  destruct H as [Hv | Hs].
  - (* P tru (C 0) 是值：矛盾 *)
    apply H1. exact Hv.
  - (* P tru (C 0) 可以步进：矛盾 *)
...
Proof.
  ...
  - (* 求值条件 Y = 0 *)
    ...
    + eapply multi_step.
      ...
      * (* simpl 会将 st Y =? 0 计算为具体值 *)
        replace (st Y =? 0) with true.
        2: { symmetry. apply Nat.eqb_eq. exact HY. }
        simpl. 
        eapply multi_step.
        -- apply CS_Par2. apply CS_IfTrue.
        -- (* 执行 then 分支 *)
...
Proof.
  intros n st [HX HY].
  (* 我们通过对 n 进行归纳来构造所需的状态 *)
  induction n as [| n' IHn'].
  
  - (* n = 0 的情况 *)
    (* 状态 st 本身就满足条件，因为 st X = 0 *)
    exists st.
    split.
    + apply multi_refl. (* 不需要任何步骤 *)
    + split.
      * exact HX. (* st X = 0 *)
      * exact HY. (* st Y = 0 *)
      
  - (* n = S n' 的情况 *)
    (* 根据归纳假设，存在状态 st'' 使得 st'' X = n' 且 st'' Y = 0 *)
    destruct IHn' as [st'' [Hmulti [HX'' HY'']]].
    
    (* 使用 par_body_n__Sn 引理，从 st'' 到达 X = S n' 的状态 *)
    pose (st' := X !-> S n'; st'').
    exists st'.
    split.
    + (* 证明 par_loop / st -->* par_loop / st' *)
      eapply multi_trans.
      * exact Hmulti. (* st -->* st'' *)
      * (* 应用 par_body_n__Sn: st'' -->* st' *)
        apply par_body_n__Sn.
        split.
        -- exact HX''. (* st'' X = n' *)
        -- exact HY''. (* st'' Y = 0 *)
    + (* 证明 st' X = S n' 且 st' Y = 0 *)
      split.
      * (* st' X = S n' *)
        unfold st'. 
        rewrite t_update_eq.
