...
Inductive tm : Type :=
  | tru : tm                    (* 布尔值 true *)
  | fls : tm                    (* 布尔值 false *)
  | ite : tm -> tm -> tm -> tm  (* if-then-else 条件表达式 *)
  | zro : tm                    (* 自然数 0 *)
  | scc : tm -> tm              (* 后继函数 successor *)
  | prd : tm -> tm              (* 前驱函数 predecessor *)
  | iszro : tm -> tm.           (* 零判断函数 *)
...
Proof.
  ...
  -unfold step_normal_form. intros R. inversion R. inversion H. subst. inversion H1.
  -unfold value. intros [H|H].
   -- inversion H.
...
Proof.
  intros t H. intros [t' Hstep].
  destruct H as [Hbvalue | Hnvalue].
  - (* bvalue case *)
    inversion Hbvalue; subst; inversion Hstep.
  - (* nvalue case *)
    induction Hnvalue; inversion Hstep; subst.
    + (* t = succ t0, step must be by ST_Succ *)
      admit.
Admitted.
...
Proof with eauto.
  ...
  induction Hy1; intros y2 Hy2; inversion Hy2; subst; auto.
  - (* ST_If vs ST_If *)
    inversion H3. 
  - (* ST_If vs ST_IfTrue - contradiction *)
    inversion H3.
  - (* ST_If vs ST_IfFalse - contradiction *)
    inversion Hy1.
  - (* ST_IfTrue vs ST_If - contradiction *)
    inversion Hy1.
  - (* ST_IfFalse vs ST_If - contradiction *)
    f_equal. apply IHHy1. assumption.
  - (* ST_Succ vs ST_Succ *)
    f_equal. apply IHHy1. assumption.
  - (* ST_PredSucc vs ST_PredSucc *)
    inversion H0.
  - (* ST_PredSucc vs ST_Pred - contradiction: succ v cannot step *)
    (* assert P as H by *)
    assert (value <{ succ v }>) as Hval by (right; apply nv_succ; assumption).
    apply value_is_nf in Hval. unfold step_normal_form in Hval.
    exfalso. apply Hval. exists t1'. assumption.
  - (* ST_Pred vs ST_PredSucc - contradiction: succ v cannot step *)inversion Hy1.
  - (* ST_Pred vs ST_Pred *)
    assert (value <{ succ y2 }>) as Hval by (right; apply nv_succ; assumption).
    apply value_is_nf in Hval. unfold step_normal_form in Hval.
    exfalso. apply Hval. exists t1'. assumption.
  - (* ST_IsZeroSucc vs ST_IsZero - contradiction: succ v cannot step *)
    f_equal. apply IHHy1. assumption.
  - (* ST_IsZero vs ST_IsZeroSucc - contradiction: succ v cannot step *)
    inversion H0.
  - (* ST_IsZero vs ST_IsZero *)
    assert (value <{ succ v }>) as Hval by (right; apply nv_succ; assumption).
    apply value_is_nf in Hval. unfold step_normal_form in Hval.
    exfalso. apply Hval. exists t1'. assumption.
  - (* ST_IsZero0 vs ST_IsZero - contradiction: 0 cannot step *)
    inversion Hy1.
  - assert (value <{ succ v }>) as Hval by (right; apply nv_succ; assumption).
      apply value_is_nf in Hval. unfold step_normal_form in Hval.
    exfalso. apply Hval. exists t1'. assumption.
...
Proof.
  intros Contra.
  inversion Contra. subst.  (* 第一次 inversion *)
  inversion H4.   
...
Proof.
  ...
  - (* T_Succ *)
    ...
    + left. right. apply nv_succ.
      ...
      -- inversion Hb; subst; inversion HT.
      -- assumption.
    + right. destruct H as [t' Hs]. exists <{ succ t' }>. auto.
  - (* T_Pred *)
    destruct IHHT.
    + (* t1 is a value *)
      right. (* 选择 exists t', t --> t' 分支 *)
    ...
    + (* t1 can take a step *)
      right. destruct H as [t1' Hs].
      exists <{ pred t1' }>. apply ST_Pred. assumption.
  - (* T_Iszero *)
    destruct IHHT.
    * (* t1 is a value *)
    ...
    * (* t1 can take a step *)
      right. destruct H as [t1' Hs].
      exists <{ iszero t1' }>. apply ST_IsZero. assumption.
Qed.
  (* FILL IN HERE *)
(** [] *)
...
Proof with eauto.
  ...
  - (* ST_Succ *) apply T_Succ. apply IHHE. assumption.
  - (* ST_Pred0 *) apply T_0.
  - (* ST_PredSucc *) inversion H1. assumption.
  - (* ST_Pred *) apply T_Pred. apply IHHE. assumption.
  - (* ST_IsZero0 *) apply T_True.
  - (* ST_IsZeroSucc *) inversion H1. apply T_False.
  - (* ST_IsZero *) apply T_Iszero. apply IHHE. assumption.
Qed.
  (* FILL IN HERE *) 
(** [] *)
...
Proof.
  right. (* 选择否定，即主语扩展不成立 *)
  intros H.
  (* 构造反例：if true then 0 else true --> 0 *)
  assert (Hstep: <{ if true then 0 else true }> --> <{ 0 }>) by apply ST_IfTrue.
  assert (Htype: <{ |-- 0 \in Nat }>) by apply T_0.
  (* 应用假设 H *)
  assert (Hcontra: <{ |-- if true then 0 else true \in Nat }>) by 
    (apply (H <{ if true then 0 else true }> <{ 0 }> Nat); split; [exact Hstep | exact Htype]).
  (* 但是 if true then 0 else true 不能有任何类型 *)
  inversion Hcontra; subst.
  (* H4: |-- true \in Bool, H5: |-- 0 \in Nat, H6: |-- true \in Nat *)
  inversion H6. (* true 不能有 Nat 类型，矛盾 *)
Qed.
(** [] *)
